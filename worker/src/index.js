/**
 * Пилюлькин День — Push-сервер на Cloudflare Workers (Web Push, чистый Web Crypto).
 *
 * Эндпоинты:
 *   GET  /api/vapid-public-key — публичный VAPID-ключ
 *   POST /api/subscribe        — { subscription, pills?, tzOffsetMin? }
 *   POST /api/unsubscribe      — { endpoint }
 *   POST /api/pills/save       — { pills, tzOffsetMin }
 *   POST /api/test-push        — тестовое уведомление всем
 *   GET  /api/debug            — состояние
 *
 * Cron: каждую минуту проверяет расписание и рассылает push.
 *
 * Переменные/секреты:
 *   VAPID_PUBLIC_KEY  — публичный ключ, base64url (65 байт)
 *   VAPID_PRIVATE_KEY — приватный ключ. Принимаются ОБА формата:
 *                       а) полная JWK JSON-строка {"kty":"EC",...} (как выдаёт tools/gen_vapid.js)
 *                       б) base64url-строка "d" (43 символа)
 *   VAPID_SUBJECT     — mailto:email
 *   SITE_URL          — URL сайта (для ссылки в уведомлении)
 */
...
function parsePrivateJwk(env) {
  ...
  // Формат а) полная JWK JSON-строка
  if (raw.startsWith("{")) {
    const jwk = JSON.parse(raw);
    if (!jwk.d || !jwk.x || !jwk.y) throw new Error("JWK must contain x, y, d");
    return {
      // Оставляем только нужные поля — key_ops/use из Node-экспорта могут мешать importKey
      kty: "EC",
      crv: "P-256",
      x: String(jwk.x),
      y: String(jwk.y),
  ...
  // Формат б) base64url "d" — x/y берём из публичного ключа
  const pubBytes = b64urlDecode((env.VAPID_PUBLIC_KEY || "").trim());
  if (pubBytes.length !== 65 || pubBytes[0] !== 4) {
    throw new Error("VAPID_PUBLIC_KEY must be 65-byte uncompressed point (base64url)");
  }
  return {
    kty: "EC",
    crv: "P-256",
...
async function vapidAuthHeader(endpoint, env) {
  ...
  return `vapid t=${unsigned}.${b64urlEncode(new Uint8Array(sigBuf))}, k=${(env.VAPID_PUBLIC_KEY || "").trim()}`;
}
...
// Поддерживаем оба варианта имени биндинга: PILLS или KV_NAMESPACE
function kv(env) {
  return env.PILLS || env.KV_NAMESPACE;
}

async function getSubscriptions(env) {
  const store = await kv(env).get("subscriptions", "json");
  return (store && store.subscriptions) || [];
}

async function saveSubscriptions(env, subs) {
  await kv(env).put("subscriptions", JSON.stringify({ subscriptions: subs }));
}

// ===== KV: расписание =====
async function getSchedule(env) {
  const rec = await kv(env).get("user:pills", "json");
  return rec || { pills: [], tzOffsetMin: 0, notifiedToday: {} };
}

async function saveSchedule(env, rec) {
  await kv(env).put("user:pills", JSON.stringify(rec));
}
...
async function handleRequest(request, env) {
  ...
  if (url.pathname === "/api/vapid-public-key" && method === "GET") {
    return new Response((env.VAPID_PUBLIC_KEY || "").trim(), {
  ...
  if (url.pathname === "/api/test-push" && method === "POST") {
    ...
    for (const rec of subs) {
      ...
      if (result.ok) sent++;
        ...
        if (result.status === 404 || result.status === 410) removed.push(rec.subscription.endpoint);
        else errors.push({ status: result.status, error: result.error, endpoint: rec.subscription.endpoint.slice(0, 60) });
      }
    }
    if (removed.length) {
      await saveSubscriptions(env, subs.filter(s => !removed.includes(s.subscription.endpoint)));
    }
    return json({ success: sent > 0, sent, removed: removed.length, errors });
  }
  ...
  if (url.pathname === "/api/debug" && method === "GET") {
    ...
    return json({
      ...
      privateKeyFormat: (env.VAPID_PRIVATE_KEY || "").trim().startsWith("{") ? "jwk-json" : "base64url-d",
